...
class Loader implements TmsLoader {
	...
	constructor() {
		...
		(window as any)["logs"] = Object.assign(this.#logs, {
			...
			stack: function (label: string) {
				const item = Array.prototype.find.call(
					this,
					(e) => e.label === label
				);

				if ("msg" in item && "stack" in item.msg) return item.stack;
			},
		});
	}
	...
	#loadRepository(repository: Pmd["tms"]) {
		const repo: Tms[] = [];
		...
		for (let k of ["preset", "current"]) {
			...
			if (k === "current" && resolved.length <= 0)
				if (repository.pages)
					resolved.push(
						...(repository.pages[nt.data.params.pagename] || [])
					);
	...
	#executeTms(
		...
		log: TmsLog = this.#initLog()
	) {
		try {
			if ("callback" in tms || "observe" in tms) {
				// execute [tms.callback]
				if (tms.callback) tms.callback();

				// observer if needed
				if (tms.observe) nt.observers.push(tms.observe);

				// change log state
				log.state = 1;
			} else
				throw `Missing the following properties ['observer', 'callback'] on Tms: "${
					(tms as Tms).label
				}"`;
		} catch ({content, stack}) {
			Object.assign(log, {
		...
		if (!skipReport) {
			...
			if (log.state === -1 && tms.priority === 3) throw log.msg;
	...
	#executeCurrentQueue(log: boolean = false): Promise<void> {
		const resolve = (rs: any, rj: any) => {
			for (let tms; (tms = this.#queue.shift()); ) {
				...
				if (this.#resolveDepencies(tms, log)) continue;
	...
	#resolveDepencies(tms: Tms, log: TmsLog): boolean | undefined {
		const dependencies = tms.depend_on;

		if (!(dependencies instanceof Array) || dependencies.length <= 0)
			return;

		if (tms.observe) dependencies.unshift("dom_mutation_observer");
		...
		for (let dp_name; (dp_name = dependencies.shift()); ) {
			...
			if (found) {
				if (found.state === 1) continue;

				throw new CriticalError({
					log:
						found.msg ||
						`[${found.label}] as Tms dependencie had failed to run !`,
				});
			} else {
	...
	#resolveAttach(tms: Tms) {
		if (!tms.attach || !(tms.attach instanceof Array)) return;

		for (let label; (label = tms.attach.pop()); ) {
			const tms = fn.data.find(this.#queue, { label });

			if (tms) this.#queue.unshift(tms);
		}
	}

	#resolveFallback(tms: Tms) {
		...
		if (fall) {
			if (!(fall instanceof Function)) {
				if (
					(typeof fall === "boolean" && fall === true) ||
					("options" in fall && fall.options === true)
				)
					this.#queue.push(
						Object.assign(tms, {
							fallback: (fall as TmsFallback).callback,
						})
					);
					...
					if (state[0] || state[1])
						this.#RetryTms(
							Object.assign(tms, { fallback: undefined }),
							{
								...((state[1]
									? fall
									: { options: fall }) as TmsFallback),
							}
						);
				}
	...
	async #RetryTms(tms: Tms, fall: TmsFallback) {
		const op = fall.options as TmsFallbackRetryOptions;

		let retry_limit = op.n || 1;

		if (retry_limit < 0 || retry_limit > 3) return;
		...
		const resolve = () => {
			...
			if (log.state === 1) return this.#report(log);

			if (--retry_limit > 0) {
				if (retry_limit > 1) log.state = 2;

				this.#report(log);

				setTimeout(resolve, op.delay);
			} else {
				if (fall.callback instanceof Function) fall.callback();

				if (tms.priority === 3) throw log.msg;
			}
		};
	...
	#report(log: TmsLog) {
		...
		Object.assign(log.t, { end, runtime: end - log.t.start });

		if (log.label !== "") this.#logs.push(log);
	}
}
